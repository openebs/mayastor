...
async fn nvmf_device_unmap_blocks() {
    ...
    // Read completion callback.
    fn unmap_completion_callback(success: bool, ctx: *const c_void) {
        // Make sure callback is invoked only once.
        flag_callback_invocation();

        assert!(success, "unmap_blocks() failed");
        // Make sure we were passed tha same pattern string as requested.
        let s = unsafe {
            let slice = slice::from_raw_parts(
                ctx as *const u8,
    ...
    println!("Sleeping for 2 secs to let unmap operation complete");
    tokio::time::delay_for(std::time::Duration::from_secs(3)).await;
    println!("Awakened.");
