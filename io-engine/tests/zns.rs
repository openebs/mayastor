...
use io_engine::{
    bdev::nexus::{nexus_bdev_error::Error, nexus_create, nexus_lookup_mut},
    core::{MayastorCliArgs, Protocol, UntypedBdevHandle},
};

pub mod common;
use common::compose::MayastorTest;
use io_engine_tests::NullBlk;
use run_script::{self};
...
fn prepare_storage() -> NullBlk {
    common::delete_file(&[DISKNAME2.into()]);
    common::truncate_file(DISKNAME2, 64 * 1024);
    let ret =
        common::create_zoned_nullblk_device(4096, 2048, 1077, 0, 16, 14, 14);
...
async fn zns_fio() {
    ...
    let _nullblk_id = prepare_storage();
...
async fn zns_replicated() {
    ...
    let _nullblk_id = prepare_storage();
    let ret = ms.spawn(async { create_replicated_nexus().await }).await;
