...
//! [`read_device_health`] is called from an SPDK reactor (the `ListPoolsSMART`
...
/// over `ListPoolsSMART`'s gRPC response as `DeviceHealth::error_log_entries`
...
/// thread (the normal case -- `ListPoolsSMART` runs inside `spdk_submit!`),
...
async fn run_smartctl(path: &str) -> Result<DeviceHealth, CoreError> {
    let output = Command::new("smartctl")
        ...
        .map_err(|error| {
            ...
            // log line) so it's visible to the ListPoolsSMART caller too, via
