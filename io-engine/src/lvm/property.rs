...
impl Property {
    ...
    pub(super) fn value(&self) -> Option<String> {
        match self {
            Property::Lvm => None,
            Property::LvName(name) => Some(name.to_string()),
            Property::LvShare(protocol) => {
                Some(protocol.value_str().to_string())
            }
            Property::LvAllowedHosts(hosts) => {
                Some(hosts.join(",").to_string())
            }
            Property::Unknown(_, value) => Some(value.to_string()),
        }
    }
    ...
    fn new_known(key: &str, value: &str) -> Option<Self> {
        match PropertyType::from_str(key).ok()? {
            ...
            PropertyType::LvName => Some(Self::LvName(value.to_string())),
