...
pub enum LabelError {
    ...
    #[snafu(display("Failed to allocate buffer for writing to child {}: {}", name, source))]
    ...
    #[snafu(display("Error reading from child {}: {}", name, source))]
    ReadError2 { source: DmaError, name: String },
    ...
    #[snafu(display("Failed to obtain BlockDeviceHandle for child {}: {}", name, source))]
    ...
    #[snafu(display("Error setting MetaDataIndex address for child {}: {}", name, error))]
    IndexAddress { error: String, name: String },
}
...
impl GptLabel {
    ...
    pub const METADATA_PARTITION_SIZE: u64 = 3 * 1024 * 1024;
    ...
    pub fn generate_label(
        ...
        let size = block_size * num_blocks;

        // Partition table
        let mut partitions = GptLabel::create_partitions(&header, block_size, size)?;
    ...
    fn create_partitions(
        ...
        partitions.push(GptEntry {
            ent_type: GptGuid::from_str(GptLabel::METADATA_PARTITION_TYPE_ID).unwrap(),
            ent_guid: GptGuid::new_random(),
            ent_start: header.lba_start,
            ent_end: data_start - 1,
            ent_attr: 0,
            ent_name: "metadata".into(),
        });
    ...
    fn check_maya_partitions(label: &GptLabel) -> bool {
        ...
        let ent_type = GptGuid::from_str(GptLabel::METADATA_PARTITION_TYPE_ID).unwrap();
...
impl GptLabel {
    ...
    fn read_partitions(buf: &DmaBuf, header: &GptHeader) -> Result<Vec<GptEntry>, ProbeError> {
...
impl NexusChild<'_> {
    ...
    pub async fn probe_label(&self) -> Result<GptLabel, LabelError> {
        let handle = self.get_io_handle().context(HandleSnafu {
        ...
        // Protective MBR
        let mut buf = handle.dma_malloc(block_size).context(ReadError2Snafu {
            name: String::from("header"),
        })?;
        handle.read_at(0, &mut buf).await.context(ReadSnafu {
            name: String::from("MBR"),
        })?;
        let mbr = GptLabel::read_mbr(&buf).context(InvalidLabelSnafu {})?;
        ...
        let mut partitions =
            GptLabel::read_partitions(&buf, active).context(InvalidLabelSnafu {})?;
    ...
    async fn create_label(&mut self, size: u64) -> Result<GptLabel, LabelError> {
        // Create new label.
    ...
    async fn update_label(&mut self, size: u64) -> Result<GptLabel, LabelError> {
        match self.probe_label().await {
            Ok(label) if GptLabel::check_maya_partitions(&label) => {
                // Keep existing label.
    ...
    async fn validate_label(&self) -> Result<GptLabel, LabelError> {
        let label = self.probe_label().await?;

        if !GptLabel::check_maya_partitions(&label) {
            return Err(LabelError::InvalidLabel {
    ...
    async fn generate_and_write_label(&mut self, size: u64) -> Result<GptLabel, LabelError> {
        let handle = self.get_io_handle().context(HandleSnafu {
        ...
        let label = GptLabel::generate_label(guid, bdev.block_len(), bdev.num_blocks())?;
    ...
    async fn check_or_write_label(&mut self, label: &GptLabel) -> Result<(), LabelError> {
        let handle = self.get_io_handle().context(HandleSnafu {
            name: self.name.clone(),
        })?;
    ...
    async fn write_label(
        ...
        match label.status {
            ...
            NexusLabelStatus::Primary => {
                ...
                let secondary = GptLabel::get_secondary_data(handle, label)?;
                handle
            ...
            NexusLabelStatus::Secondary => {
                ...
                let primary = GptLabel::get_primary_data(handle, label)?;
                handle
...
//     pub(crate) async fn validate_child_labels(&mut self) -> Result<(), LabelError> {
...
//                     NexusMetaData::get_index_lba(&label).context(IndexAddressSnafu {
...
//     pub(crate) async fn update_child_labels(&mut self) -> Result<(), LabelError> {
//         if self.children.is_empty() {
//             return Err(LabelError::MissingChildren {
//                 name: self.name.clone(),
//             });
...
//     pub(crate) async fn create_child_labels(&mut self) -> Result<(), LabelError> {
//         if self.children.is_empty() {
//             return Err(LabelError::MissingChildren {
//                 name: self.name.clone(),
//             });
...
impl GptLabel {
    ...
    fn get_primary_data(
        ...
        serialize_into(&mut writer, &label.mbr).context(SerializeSnafu)?;

        // Primary GPT header
        writer
            .seek(SeekFrom::Start(label.primary.lba_self * label.block_size))
            .unwrap();
        serialize_into(&mut writer, &label.primary).context(SerializeSnafu)?;
    ...
    fn get_secondary_data(
        ...
        let mut buf = handle
            .dma_malloc(
                (label.secondary.lba_self - label.secondary.lba_table + 1) * label.block_size,
            )
            .context(WriteAllocSnafu {
        ...
        writer
            ...
            .unwrap();
        serialize_into(&mut writer, &label.secondary).context(SerializeSnafu)?;
