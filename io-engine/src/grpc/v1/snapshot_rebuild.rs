    ...
    async fn create_snapshot_rebuild(
        ...
        let rx = rpc_submit_ext(async move {
            ...
            if let Ok(job) = SnapshotRebuildJob::lookup(&request.replica_uuid) {
                return Ok(SnapshotRebuild::from(RebuildJob::from(job).await));
            }
            ...
            let job = RebuildJob::lookup(&request.replica_uuid).await?;
            let _channel = job.job.start().await?;
            let _ = mayastor_sleep(Duration::from_millis(10)).await;
            job.job.pause().unwrap();
        ...
        let rx = rpc_submit_ext(async move {
            ...
            match args.replica_uuid {
                None => {
                    ...
                    for job in jobs {
                        rebuilds.push(RebuildJob::from(job).await.into());
                    }
                ...
                Some(uuid) => {
                    let job = RebuildJob::lookup(&uuid).await?;
                    Ok(ListSnapshotRebuildResponse {
...
impl From<RebuildJob> for SnapshotRebuild {
    fn from(job: RebuildJob) -> Self {
        let stats = job.stats;
        let job = job.job;
        Self {
            uuid: job.dst_uri().to_string(),
