...
@pytest.fixture
def mayastor_nexus(mayastor_instance, nexus_name, nexus_uuid, source_uri):
...
def rebuild_state(mayastor_instance, mayastor_nexus, nexus_uuid, target_uri):
    try:
        yield mayastor_instance.nexus_rpc.GetRebuildState(
            pb.RebuildStateRequest(nexus_uuid=nexus_uuid, uri=target_uri)
        ).state
    except:
        yield None
...
def start_rebuild(mayastor_instance, mayastor_nexus, nexus_uuid, target_uri):
    mayastor_instance.nexus_rpc.StartRebuild(
        pb.StartRebuildRequest(nexus_uuid=nexus_uuid, uri=target_uri)
    )


@when("the rebuild operation is then paused")
def pause_rebuild(mayastor_instance, mayastor_nexus, nexus_uuid, target_uri):
    mayastor_instance.nexus_rpc.PauseRebuild(
        pb.PauseRebuildRequest(nexus_uuid=nexus_uuid, uri=target_uri)
    )
    time.sleep(0.5)
...
def resume_rebuild(mayastor_instance, mayastor_nexus, nexus_uuid, target_uri):
    mayastor_instance.nexus_rpc.ResumeRebuild(
        pb.ResumeRebuildRequest(nexus_uuid=nexus_uuid, uri=target_uri)
    )


@when("the rebuild operation is then stopped")
def stop_rebuild(mayastor_instance, mayastor_nexus, nexus_uuid, target_uri):
    mayastor_instance.nexus_rpc.StopRebuild(
        pb.StopRebuildRequest(nexus_uuid=nexus_uuid, uri=target_uri)
    )
    time.sleep(0.5)
...
def rebuild_statistics(mayastor_instance, mayastor_nexus, nexus_uuid, target_uri):
    return mayastor_instance.nexus_rpc.GetRebuildStats(
        pb.RebuildStatsRequest(nexus_uuid=nexus_uuid, uri=target_uri)
    )
...
def set_child(mayastor_instance, mayastor_nexus, nexus_uuid, target_uri, state):
    mayastor_instance.nexus_rpc.ChildOperation(
        pb.ChildOperationRequest(
