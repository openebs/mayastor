...
@scenario('features/lvm.feature', 'Determine if the mayastor instance supports LVM pools')
...
def volgrp_with_losetup_disk():
    ...
    run_cmd(f"sudo vgcreate '{name}' '{disk}'", True)
    yield name
    run_cmd(f"rm -f '{file}'", True)
...
def get_lvm_feature(get_mayastor_instance, get_mayastor_info):
    return get_mayastor_info.supportedFeatures.lvm

@then('the instance shall report if it supports the LVM feature')
def the_instance_shall_report_if_it_supports_the_lvm_feature(get_mayastor_instance, get_mayastor_info, get_lvm_feature):
    assert get_lvm_feature == True


@when('the user creates a pool specifying a URI representing an loop disk')
def the_user_creates_a_pool_specifying_a_uri_representing_an_loop_disk(get_mayastor_instance, volgrp_with_losetup_disk, create_pool):
...
def the_user_destroys_a_pool_specifying_type_as_lvm(get_mayastor_instance):
    get_mayastor_instance.ms.DestroyPool(pb.DestroyPoolRequest(name="lvmpool"))
...
def the_user_destroys_a_pool_specifying_type_as_lvs(get_mayastor_instance):
    get_mayastor_instance.ms.DestroyPool(pb.DestroyPoolRequest(name="lvspool"))
...
def list_pools(get_mayastor_instance):
    return get_mayastor_instance.ms.ListPools(pb.Null()).pools
